...
If $\|\cdot\|$ is even a norm on $\mathbb{C}^n$, then
$$
\|M^k\|^{1/k} \ge \rho(M).
$$
...
If $\|\cdot\|$ is a norm on $\mathbb{C}^n$, let $\lambda \in \sigma(M)$ with
$$
|\lambda| = \rho(M).
$$
...
Let $B \in \{M_\pm\}$. Recall the Jordan normal form
$$
B = VJV^{-1},
$$
...
Recall that $\|\cdot\|_\infty$ induces the row-sum norm.
