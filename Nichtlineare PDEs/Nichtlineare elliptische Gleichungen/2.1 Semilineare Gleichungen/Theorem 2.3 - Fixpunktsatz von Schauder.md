...
*Behauptung: (siehe [[Nichtlineare PDEs/Nichtlineare elliptische Gleichungen/Uebung/Uebung 1/Aufgabe 1]])* Man kann $F_{j}$ so konstruieren, dass $||F_{j}(x)-x||\leq \frac{1}{j}$, daher ist $F_{j}$ eine quasi Approximation der Identität mit Wertebereich in $K_{j}$.
