...
Zu zeigen wäre:
- $f(x,u(x))v$ ist messbar. (siehe [[Nichtlineare PDEs/Nichtlineare elliptische Gleichungen/Uebung/Uebung 1/Aufgabe 2]])
...
> $$
> **Falls** wir zusätzlich wüssten ([[Nichtlineare PDEs/Nichtlineare elliptische Gleichungen/Uebung/Uebung 1/Aufgabe 3]], wir verwenden, dass der Trace-Operator $\gamma:H^1(\Omega)\to L^2(\Omega)$ stark stetig in $L^2(\Omega)$ w.r.t. schwachen Konvergenz in $H^1(\Omega)$ ist.) , dass $u_{v_{k_{n}}}=g$ auf $\partial \Omega$ $\implies u^*=g$ auf $\partial \Omega$, wüssten wir $S(v)=u^*$
